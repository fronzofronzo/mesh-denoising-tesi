...
class DGCNN(nn.Module):
    def __init__(self, k, init_dims, emb_dims, dropout, output_channels=3):
        ...
        self.conv7 = nn.Sequential(nn.Conv2d(256*2, 256, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(1280, emb_dims, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, inputs):
        ...
        x = F.leaky_relu(self.bn9(self.linear1(x)), negative_slope=0.2)
        x = self.dp1(x)
        x = F.leaky_relu(self.bn10(self.linear2(x)), negative_slope=0.2)
        x = self.dp2(x)
        x = F.leaky_relu(self.bn11(self.linear3(x)), negative_slope=0.2)
        x = self.linear4(x)
        return x
