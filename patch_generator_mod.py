...
def find_second_ring_neighbor(face_index, adj_pairs):
    # Find first ring neighbors
    first_ring_neighbors = set()
    if adj_pairs is not None and adj_pairs.size > 0:
        try:
            neighbors1 = adj_pairs[adj_pairs[:, 0] == face_index, 1]
            neighbors2 = adj_pairs[adj_pairs[:, 1] == face_index, 0]
            first_ring_neighbors = set(np.unique(np.concatenate((neighbors1, neighbors2))))
        except Exception as e:
            print(f"Error finding first ring neighbors for face {face_index}: {e}")
    
    # Find second ring neighbors (P_i^2)
    second_ring_neighbors = set()
    if adj_pairs is not None and adj_pairs.size > 0:
        for neighbor_idx in first_ring_neighbors:
            try:
                n1 = adj_pairs[adj_pairs[:, 0] == neighbor_idx, 1]
                n2 = adj_pairs[adj_pairs[:, 1] == neighbor_idx, 0]
                neighbors_of_neighbor = set(np.unique(np.concatenate((n1, n2))))
                second_ring_neighbors.update(neighbors_of_neighbor)
            except Exception as e:
                print(f"Error finding second ring neighbors from {neighbor_idx}: {e}")
        
        # Remove central face and first ring from second ring
        second_ring_neighbors.discard(face_index)
    return second_ring_neighbors

def apply_normal_filtering(normals, adj_pairs, num_iterations=1):
    """
    Apply normal filtering: n_i^{k+1} = 1 / |P^f_i| \sum_{f_j \in P_i^f} n_j^k
    where P^f_i is the set of faces that share an edge with f_i.
    
    Args:
        normals: array of face normals (N x 3)
        adj_pairs: adjacency pairs from face_adjacency
        num_iterations: number of filtering iterations
    
    Returns:
        filtered_normals: filtered normals after specified iterations
    """
    filtered_normals = np.copy(normals)

    for iteration in range(num_iterations):
        new_normals = np.copy(filtered_normals)
        for face_idx in range(len(normals)):
            adjacent_faces = []
            if adj_pairs is not None and adj_pairs.size > 0:
                neighbors1 = adj_pairs[adj_pairs[:, 0] == face_idx, 1]
                neighbors2 = adj_pairs[adj_pairs[:, 1] == face_idx, 0]
                adjacent_faces = np.unique(np.concatenate((neighbors1, neighbors2)))
            if len(adjacent_faces) > 0:
                ...
                if norm > 0:
                    ...
                    new_normals[face_idx] = filtered_normals[face_idx]
            else:
                new_normals[face_idx] = filtered_normals[face_idx]
...
def calculate_alpha_n(original_normals, filtered_normals):
    """
    Calculate alpha^n_i = acos(n_i * n_i^m)
    
    Args:
        original_normals: original noisy normals
        filtered_normals: filtered normals after normal filtering
    
    Returns:
        alpha_n: array of alpha^n angles for each face
    """
    orig_norm = np.linalg.norm(original_normals, axis=1, keepdims=True)
    orig_norm[orig_norm == 0] = 1
    original_normals_normalized = original_normals / orig_norm

    filt_norm = np.linalg.norm(filtered_normals, axis=1, keepdims=True)
    filt_norm[filt_norm == 0] = 1
    filtered_normals_normalized = filtered_normals / filt_norm

    dot_products = np.sum(original_normals_normalized * filtered_normals_normalized, axis=1)
    
    # Clamp dot products to [-1, 1] to avoid numerical issues with acos
    dot_products = np.clip(dot_products, -1.0, 1.0)
    
    # Calculate alpha^n = acos(dot_product)
    alpha_n = np.arccos(dot_products)

    return alpha_n

def calculate_alpha_g(filtered_normals, adj_pairs):
    ...
    norms[norms == 0] = 1
    filtered_normals_normalized = filtered_normals / norms
    for face_index in range(num_faces):
        second_ring_neighbor = find_second_ring_neighbor(face_index, adj_pairs)
        if len(second_ring_neighbor) == 0:
            alpha_g_values[face_index] = 0.0
            continue
        
        second_ring_list = list(second_ring_neighbor)
        second_ring_normals = filtered_normals_normalized[second_ring_list]
        max_angle = 0.0
        n_faces_ring = len(second_ring_list)

        if(n_faces_ring) >= 2:
            for i in range(n_faces_ring):
                for j in range(i+1, n_faces_ring):
                    dot_product = np.dot(second_ring_normals[i], second_ring_normals[j])
        ...
        alpha_g_values[face_index] = max_angle
    
    return alpha_g_values

class PatchData:
    def __init__(self, expanded_mesh, face_index, gt_normals, filter_num_iterations):
        ...
        filtered_normals = apply_normal_filtering(self.normals, expanded_mesh.face_adjacency, filter_num_iterations)
        self.alpha_n_values = calculate_alpha_n(self.normals, filtered_normals)
        self.alpha_g_values = calculate_alpha_g(filtered_normals, expanded_mesh.face_adjacency)
    ...
    def calc_patch(self, central_face, faces, vertices):
        ...
        second_ring_neighbors = find_second_ring_neighbor(current_face_index, adj_pairs)
    ...
    def calculate_features(self):
        ...
        patch_alpha_n_values = self.alpha_n_values[list(self.patch_faces)]
        patch_alpha_g_values = self.alpha_g_values[list(self.patch_faces)]
        min_alpha_n = np.min(patch_alpha_n_values)
        max_alpha_n = np.max(patch_alpha_n_values)
        alpha_n_range = max_alpha_n - min_alpha_n
        if alpha_n_range == 0:
            alpha_n_range = 1
        features=[]

        min_alpha_g = np.min(patch_alpha_g_values)
        max_alpha_g = np.max(patch_alpha_g_values)
        alpha_g_range = max_alpha_g - min_alpha_g
        if alpha_g_range == 0:
            alpha_g_range = 1
        for face in self.patch_faces:
            ...
            alpha_n_normalized = (self.alpha_n_values[face] - min_alpha_n) / alpha_n_range
            alpha_g_normalized = (self.alpha_g_values[face] - min_alpha_g) / alpha_g_range
            features.append(np.hstack([centroid, normal, area, neighbors, alpha_n_normalized, alpha_g_normalized, vertices.flatten()]))
...
def process_patch(face_index, expanded_mesh, output_dir, gt_normals, num_filter_iterations):
    try:
        patch = PatchData(expanded_mesh, face_index, gt_normals, num_filter_iterations)
...
if __name__ == "__main__":
    ...
    parser.add_argument('noise_level', default="0.1",
                        ...
                        type=float)
    parser.add_argument('--filter_iterations', default=1, type=int,             help="Number of normal filtering iterations for alpha^n calculation")
    ...
    os.makedirs(f"new_samples/{mesh_name}_{noise_level}")
    output_directory = os.path.join(script_dir, "new_samples", f"{mesh_name}_{noise_level}")
